...
func init() {
	if val, ok := os.LookupEnv("REMOTE_ADDR"); !ok {
		panic(errors.New("Invalid `REMOTE_ADDR` in env.."))
	} else {
	...
	if val2, ok := os.LookupEnv("REDIS_ADDR"); !ok {
		panic(errors.New("Invalid `REDIS_ADDR` in env.."))
	} else {
	...
	if val3, ok := os.LookupEnv("TOKEN2"); !ok {
		panic(errors.New("Invalid `TOKEN2` in env.."))
	} else {
		token2 = val3
	}
}
...
func (p ProxyHandler) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	if r.Header.Get("X-MelonBot-App") == token2 {
		limit = 20
		key = "token2"
	} else {
		limit = 2
